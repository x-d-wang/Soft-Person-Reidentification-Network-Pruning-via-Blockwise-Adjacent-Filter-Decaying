...
parser.add_argument('--knn',default=11,type=int, help='k for PFP')
...
def train_model(model, criterion, optimizer, scheduler, num_epochs=25):
    ...
    for epoch in range(num_epochs):
        ...
        if pretrained_dir !='' and epoch == 0: #jump the first pretrained model, cause the initial loss too small
        ...
        m.model = model
        m.if_zero()
        m.init_mask(prune_rate,decay_rate = decay_rate,k = knn_k)
        print(m.compress_rate)
        m.do_mask()
        m.if_zero()
        model = m.model
        model = model.cuda()
        if epoch >= start_save_epoch -1:
            ...
            m.do_mask()
            m.if_zero()
            save_network(m.model, str(epoch+1))
             #restore model
            #for last epoch
...
if opt.use_dense:
    ...
    model = ft_net(len(class_names),arch=opt.network_arch)
...
copyfile('./APFP_train.py', dir_name+'/APFP_train.py')
copyfile('./scripts/APFP_train.sh', dir_name+'/APFP_train.sh')
